...
class _DetailParkingPageState extends State<DetailParkingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
        create: (context) =>
            DetailParkingBloc()..checkDetailParking(retributionId.toString()),
              ...
              child: Column(
                children: [
                    ...
                    child: Row(
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                              retribution?.pelanggan?.namaLengkap ?? "-",
                              style: TextStyle(
                            ...
                            Text(
                              "ID${retribution?.pelanggan?.id.toString() ?? " - "}",
                              style: TextStyle(
                                  color: AppColors.textPassive, fontSize: 14),
                            ),
                            SizedBox(
                              height: 2,
                            ),
                            Text(
                              retribution?.pelanggan?.email ?? "-",
                              style: TextStyle(
                  ...
                  Text(
                    "No. Pol",
                    style:
                        TextStyle(color: AppColors.textPassive, fontSize: 16),
                  ),
                  Text(
                    retribution?.nopol ?? " - ",
                    style: TextStyle(
                  ...
                  (retribution?.statusParkir ==
                                  ParkingStatus.prosesPembayaran.name ||
                              retribution?.statusParkir ==
                                  ParkingStatus.prosesPembayaranAkhir.name) &&
                          retribution?.pembayaran?.status != "SUDAH DIBAYAR"
                      ? Padding(
                          ...
                          child: ButtonDefault(
                              ...
                              onTap: () async {
                                await Navigator.pushNamed(context, "/payment",
                                    arguments: retribution);
                                provider.checkDetailParking(
                                    retributionId.toString());
                              }))
                      : Container()
                ],
              ),
